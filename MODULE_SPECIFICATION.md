...
- 主动输出模块的软件安全上限取保守值，并在 README 标记“待真实样品确认”；
- 时间和计数不会超过核心默认操作时限；
- 默认设置本身不代表已经 Apply。
...
- driver timeout 应显著小于框架 operation timeout，使 Stop/Error 后仍有清理时间。
...
Trigger。任何可能绕过全局电流、compliance、功率或量程上限的每通道设置都必须由
Backend 拒绝。
