...
- Series 3700A 用户/参考手册：``channel.open``、``channel.exclusiveclose``、
  ``channel.getclose`` 和 ``localnode.model``。3706A 使用区分大小写的 TSP，
  不能复用 7001 的 SCPI 路由命令。
...
class Keithley6221Delta3706ABackend(ModuleBackend):
    ...
    _CLOSED_ROUTE_TOKEN = re.compile(
        r"^([1-6][0-9]{3})"
        r"(?:\(([1-6][0-9]{3})\))?$"
    )
    ...
    def __init__(
        ...
        *,
        ...
        self.transport_3706a: InstrumentTransport | None = None
        self.identity_6221 = ""
        self.identity_2182a = ""
        self.identity_3706a = ""
    ...
    def _test_connections(
        ...
        test_3706a: InstrumentTransport | None = None
        try:
            ...
            if str(settings["resource_3706a"]).strip():
                test_3706a = self._transport_factory(
                    str(settings["resource_3706a"]),
                    float(settings["io_timeout_seconds"]),
                )
                identity_3706a = test_3706a.query(
                    "print(localnode.model)"
                ).strip()
                self._validate_3706a_identity(
                    identity_3706a
                )
            ...
            for transport in (test_3706a, test_6221):
                if transport is not None:
                    try:
                        transport.close()
                    except Exception:
    ...
    def _connect_3706a(
        ...
        self._close_transport_3706a()
        resource = str(settings["resource_3706a"])
        try:
            self.transport_3706a = self._transport_factory(
                resource,
                float(settings["io_timeout_seconds"]),
            )
            identity = self.transport_3706a.query(
                "print(localnode.model)"
            ).strip()
            self._validate_3706a_identity(identity)
        except ModuleError:
            ...
            self._close_transport_3706a()
            raise ModuleError(
                "Keithley 3706A was present during Enable but "
                "could not be connected for Apply",
                "K6221_3706_SWITCHER_CONNECTION_FAILED",
                resource,
            ) from exc
        self.identity_3706a = identity
        context.checkpoint()
    ...
    def _validate_identity(
        ...
        if "KEITHLEY" not in normalized or model not in normalized:
            raise ModuleError(
                ...
                "K6221_3706_IDENTITY_MISMATCH",
                identity,
            )

    @staticmethod
    def _validate_3706a_identity(identity: str) -> None:
        """严格验证 3706/3706A 系列 TSP 型号回读。

        3706A 可以在兼容模式下通过 ``localnode.model`` 报告 ``3706``；
        带 S/NFP 后缀的正式 3706A 机型也属于同一命令集。除此之外的任意
        TSP 仪表都不能被误当成切换器。
        """
        ...
        if not supported:
            raise ModuleError(
                "Expected Keithley 3706/3706A, received "
                f"{identity!r}",
                "K6221_3706_IDENTITY_MISMATCH",
                identity,
            )
    ...
    def _verify_2182a(
        ...
        if "KEITHLEY" not in normalized or "2182A" not in normalized:
            raise ModuleError(
                ...
                "K6221_3706_2182A_IDENTITY_MISMATCH",
                identity,
            )
    ...
    def _trigger_and_read(
        ...
        if completion.strip() not in {"1", "+1"}:
            raise ModuleError(
                ...
                f"{completion!r}",
                "K6221_3706_MEASUREMENT_NOT_COMPLETE",
            )
    ...
    def _switch_channel(
        ...
        if not self.switcher_available:
            if channel != "ch1":
                raise ModuleError(
                    "3706A is unavailable; only CH1 may be measured",
                    "K6221_3706_SWITCHER_UNAVAILABLE",
                    channel,
                )
        ...
        if self.transport_3706a is None:
            raise ModuleError(
                "3706A transport is not connected",
                "K6221_3706_SWITCHER_NOT_CONNECTED",
            )

        # Break-before-make：先打开全部槽位，并用 getclose("allslots")
        # 确认整台主机没有任何遗留闭合触点；只查询本配置内的通道会漏掉未知路由。
    ...
    def _expect_closed_routes(
        ...
        *,
        ...
        """解析 ``channel.getclose`` 并要求整机闭合路由精确匹配。

        四线模式可能把配对触点写成 ``1001(1031)``；解析时把主触点和配对
        触点都展开后再比较。任何未知文本、重复项、多余闭合或缺失闭合都按
        路由状态不确定处理，绝不允许继续触发电流。
        """
        ...
        if stripped:
            for raw_token in stripped.split(","):
                ...
                matched = cls._CLOSED_ROUTE_TOKEN.fullmatch(
                    token
                )
                if matched is None:
                    raise ModuleError(
                        f"{command} returned an invalid "
                        f"closed-channel list: {reply!r}",
                        "K6221_3706_SWITCHER_VERIFY_FAILED",
                        command,
                    )
                actual.append(matched.group(1))
                paired = matched.group(2)
                if paired is not None:
                    actual.append(paired)
        actual_set = frozenset(actual)
        if len(actual) != len(actual_set) or actual_set != expected:
            raise ModuleError(
                f"{command} left closed routes "
                f"{sorted(actual_set)!r}; expected "
                f"{sorted(expected)!r}",
                "K6221_3706_SWITCHER_VERIFY_FAILED",
                command,
            )
    ...
    def _enter_safe_state(
        ...
        if (
            ...
            and self.transport_3706a is None
        ):
            self.armed = False
            self.active_channel = ""
            return
        ...
        if self.switcher_available:
            if self.transport_3706a is None:
                raise ModuleError(
                    "Cannot confirm 3706A routes are open because "
                    "the switcher is not connected",
                    "K6221_3706_SAFE_STATE_FAILED",
                    "3706A",
                )
            self._write_3706a(
                'channel.open("allslots")',
    ...
    def _verify_zero_current(
        ...
        if not self._is_zero_current(current):
            raise ModuleError(
                ...
                "a route change",
                "K6221_3706_NONZERO_DURING_SWITCH",
                self.active_channel,
            )
    ...
    def _best_effort_safe_state(
        ...
        if (
            ...
            and self.transport_3706a is not None
        ):
            try:
                self.transport_3706a.write(
                    'channel.open("allslots")'
                )
            except Exception:
                pass
    ...
    def _write_3706a(
        ...
        try:
            ...
            raise ModuleError(
                f"3706A command failed: {command}: "
                f"{type(exc).__name__}: {exc}",
    ...
    def _enabled_channels(
        ...
        if not self.switcher_available:
            if not bool(settings["channels"]["ch1"]["enabled"]):
                raise ModuleError(
                    "Enable CH1 when 3706A is unavailable",
                    "K6221_3706_INVALID_SETTINGS",
    ...
    def _require_applied(self) -> dict[str, Any]:
        if self.applied_settings is None:
            raise ModuleError(
                ...
                "K6221_3706_SETTINGS_NOT_APPLIED",
            )
        if self.transport_6221 is None:
            raise ModuleError(
    ...
    def _require_ready(self) -> dict[str, Any]:
        ...
        if not self.sequence_active:
            raise ModuleError(
                ...
                "K6221_3706_SEQUENCE_NOT_ACTIVE",
            )
        return settings
    ...
    def _close_transport_3706a(self) -> None:
        if self.transport_3706a is not None:
            try:
                self.transport_3706a.close()
            finally:
                self.transport_3706a = None
    ...
    def _close_transports(self) -> None:
        ...
        if first_error is not None:
            raise ModuleError(
                ...
                "K6221_3706_RESOURCE_RELEASE_FAILED",
            ) from first_error
    ...
    def _normalized_settings(
        ...
        *,
        ...
        for key in (
            ...
            result[key] = value
        if (
            result["resource_6221"]
            and result["resource_3706a"]
            and result["resource_6221"].casefold()
            == result["resource_3706a"].casefold()
        ):
            raise ModuleError(
                "Keithley 6221 and 3706A must use different "
                "VISA resources",
                "K6221_3706_INVALID_SETTINGS",
                "resource_3706a",
            )
        ...
        if require_measurement:
            ...
            if ch1_only and not result["channels"][
                ...
                raise ModuleError(
                    "Enable CH1 because 3706A is unavailable",
                    "K6221_3706_INVALID_SETTINGS",
                    "channels.ch1.enabled",
                )
            if not enabled:
