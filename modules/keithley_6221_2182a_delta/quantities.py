"""设置界面和后端共用的 SI 前缀数值解析。

实验人员常用 ``1m``、``20u``、``500n``、``1p`` 直接表达 10 的三次幂，而普通
``QDoubleSpinBox`` 会把极小值显示成一长串零或直接舍入。本模块把这些量保留为短文本：

- 电流输入 ``1m``、``1mA``、``1e-3`` 都表示 1 mA；
- 延时输入 ``2m``、``2ms`` 都表示 2 ms；
- ``u``、``µ`` 和 ``μ`` 都视为 micro；
- 大小写有意义：``m`` 是 milli，``M`` 是 mega。

后端始终重新解析并检查边界，界面的格式化不承担安全验证。
"""

from __future__ import annotations

import math
import re
from typing import Final


_NUMBER = re.compile(
    r"""
    ^\s*
    (?P<number>
        [+-]?
        (?:
            (?:\d+(?:\.\d*)?)
            |
            (?:\.\d+)
        )
        (?:[eE][+-]?\d+)?
    )
    \s*
    (?P<prefix>[fpnumkKMGTµμ]?)
    \s*
    (?P<unit>[A-Za-zΩΩ]*)
    \s*$
    """,
    re.VERBOSE,
)

PREFIX_FACTORS: Final[dict[str, float]] = {
    "": 1.0,
    "f": 1.0e-15,
    "p": 1.0e-12,
    "n": 1.0e-9,
    "u": 1.0e-6,
    "µ": 1.0e-6,
    "μ": 1.0e-6,
    "m": 1.0e-3,
    "k": 1.0e3,
    "K": 1.0e3,
    "M": 1.0e6,
    "G": 1.0e9,
    "T": 1.0e12,
}

_EXPONENT_PREFIX: Final[dict[int, str]] = {
    -15: "f",
    -12: "p",
    -9: "n",
    -6: "u",
    -3: "m",
    0: "",
    3: "k",
    6: "M",
    9: "G",
    12: "T",
}

_UNIT_ALIASES: Final[dict[str, frozenset[str]]] = {
    "A": frozenset({"a", "amp", "amps", "ampere", "amperes"}),
    "V": frozenset({"v", "volt", "volts"}),
    "s": frozenset({"s", "sec", "secs", "second", "seconds"}),
}


def parse_quantity(
    value: object,
    *,
    expected_unit: str = "",
) -> float:
    """把数字或带 SI 前缀的短文本转换为基本 SI 单位。

    ``expected_unit`` 只在用户显式写出单位时检查；例如电流框中的 ``1m`` 可以省略
    ``A``，但 ``1mV`` 会被拒绝，避免把电压误当成电流。
    """

    if isinstance(value, bool):
        raise ValueError("boolean is not a numeric quantity")
    if isinstance(value, (int, float)):
        result = float(value)
        if not math.isfinite(result):
            raise ValueError("quantity must be finite")
        return result
    if not isinstance(value, str):
        raise ValueError("quantity must be a number or text")
    matched = _NUMBER.fullmatch(value)
    if matched is None:
        raise ValueError(
            "use a number such as 1m, 500u, 1p or 1e-6"
        )
    explicit_unit = matched.group("unit")
    if explicit_unit and expected_unit:
        allowed = _UNIT_ALIASES.get(
            expected_unit,
            frozenset({expected_unit.casefold()}),
        )
        if explicit_unit.casefold() not in allowed:
            raise ValueError(
                f"expected {expected_unit}, got {explicit_unit}"
            )
    result = (
        float(matched.group("number"))
        * PREFIX_FACTORS[matched.group("prefix")]
    )
    if not math.isfinite(result):
        raise ValueError("quantity must be finite")
    return result


def format_quantity(
    value: object,
    *,
    expected_unit: str = "",
) -> str:
    """返回紧凑工程计数文本，优先使用 ``p/n/u/m/k/M``。

    已经是合法文本的值也会先换算再规范化，因此加载 ``0.001 A`` 后显示为 ``1m``；
    极小电流不会被扩展成难以辨认的长小数。
    """

    number = parse_quantity(value, expected_unit=expected_unit)
    if number == 0:
        return "0"
    exponent = int(math.floor(math.log10(abs(number)) / 3.0) * 3)
    exponent = max(min(exponent, 12), -15)
    prefix = _EXPONENT_PREFIX[exponent]
    scaled = number / (10.0 ** exponent)
    text = f"{scaled:.9g}"
    if text == "-0":
        text = "0"
    return f"{text}{prefix}"


__all__ = [
    "PREFIX_FACTORS",
    "format_quantity",
    "parse_quantity",
]
