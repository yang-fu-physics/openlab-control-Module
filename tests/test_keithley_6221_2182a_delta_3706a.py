...
class _FakeVisaState:
    ...
    def __init__(self) -> None:
        ...
        self.switch_model = "3706A"
        self.closed_routes_reply_override: str | None = None
    ...
    def factory(
        ...
        if "::7::" in resource:
            return _Fake3706A(self, resource)
...
class _Fake3706A:
    ...
    def write(self, command: str) -> None:
        ...
        # TSP 命令区分大小写，fake 故意按手册拼写精确匹配，避免测试把
        # 大小写错误的命令当作真实仪表会接受。
        if command == 'channel.open("allslots")':
            self.state.closed_routes.clear()
        elif command.startswith(
            'channel.exclusiveclose("'
        ):
            self.state.closed_routes = set(
                self._routes(command)
            )
        else:
            raise AssertionError(
                f"Unexpected 3706A write: {command}"
            )
    ...
    def query(
        ...
        if command == "print(localnode.model)":
            return self.state.switch_model
        if command == (
            'print(channel.getclose("allslots"))'
        ):
            if (
                self.state.closed_routes_reply_override
                is not None
            ):
                return (
                    self.state.closed_routes_reply_override
            ...
            return ",".join(
                sorted(self.state.closed_routes)
            )
        raise AssertionError(
            f"Unexpected 3706A query: {command}"
        )
...
class DefaultSettingsTests(unittest.TestCase):
    def test_defaults_are_fresh_and_output_safe(
        self,
    ) -> None:
        ...
        first["channels"]["ch2"]["enabled"] = True
        self.assertFalse(
            second["channels"]["ch2"]["enabled"]
        )
        self.assertEqual(first["shared"]["high_current"], "0")
...
class BackendTests(unittest.TestCase):
    ...
    def test_open_reads_registry_and_apply_connects_selected_3706a(
        self,
    ) -> None:
        state = _FakeVisaState()
        messages: list[tuple[str, dict]] = []
    ...
    def test_selected_3706a_connection_failure_is_fatal(
        ...
        self.assertEqual(
            sum(
                action == "write"
                and command.startswith("channel.")
    ...
    def test_shared_mode_arms_once_at_sequence_start_and_emits_raw_rows(
        ...
        self.assertFalse(
            any(command.startswith("ROUT:") for command in switch_writes)
        )
        arm_commands = [
            command
            for resource, action, command
    ...
    def test_unexpected_closed_route_is_fatal_before_trigger(
        self,
    ) -> None:
        state = _FakeVisaState()
        backend = self._backend(state, [])
        ...
        state.closed_routes_reply_override = (
            "1001,1005,1006,1011,1015"
        )
